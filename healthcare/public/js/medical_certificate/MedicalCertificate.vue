<template>
  <div>
    <div
      ...
      class="medical-certificate-content"
    ...
    <div
      class="form-section"
      style="margin-top: 30px; padding: 20px; border-top: 1px solid #ddd"
    >
      <div class="form-group" style="margin-bottom: 15px">
        <label
          ><input type="checkbox" v-model="forLeave" /> For leave
          purposes</label
        >
      </div>
  ...
  methods: {
    async saveMedicalCertificate() {
      ...
      try {
        ...
        const patientId = this.patient?.name;

        if (!patientId) {
          frappe.msgprint(
            __("Patient information is missing. Please try again.")
          );
          this.saving = false;
          return;
        }
        ...
        frappe.msgprint(
          __("Error saving Medical Certificate. Please try again.")
        );
      } finally {
        this.saving = false;
      }
