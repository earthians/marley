			...
			() =>
				frappe.call({
					...
					callback: function (r) {
						...
						let observation_html = frappe.render_template("observation", {
			...
			var d = new frappe.ui.Dialog({
				...
				fields: [
					...
					},
					{
						...
						fieldtype: "Float",
						depends_on:
							"eval:['Quantity', 'Numeric'].includes(doc.permitted_data_type)",
					},
					{
						label: "Result Data",
						...
						fieldtype: "Data",
						depends_on:
							"eval:['Range', 'Ratio'].includes(doc.permitted_data_type)",
					},
				],
				primary_action: function () {
					...
					if (["Range", "Ratio"].includes(data.permitted_data_type)) {
						...
						["Quantity", "Numeric"].includes(data.permitted_data_type)
					) {
			...
			d.fields_dict["observation_template"].df.onchange = () => {
				if (d.get_value("observation_template")) {
					frappe.db.get_value(
						...
						{ name: d.get_value("observation_template") },
						["permitted_data_type", "has_component"],
						r => {
