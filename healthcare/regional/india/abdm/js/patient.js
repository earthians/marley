				...
				() =>
					frappe.db
						.get_value(
							"Patient",
							{
								abha_number: d.get_value("healthid"),
								name: ["!=", frm.doc.name],
							},
							["name", "abha_card"],
						)
						.then(r => {
							if (r.message.name) {
								...
								if (r.message.abha_card) {
									frappe.throw({
										message: `<img src='${r.message.abha_card}'>`,
										title: __("Patient already exist"),
									});
								} else {
									frappe.throw({
										message: `<a href="/app/patient/${r.message.name}">${r.message.name}</a>`,
										title: __("Patient already exist"),
									});
								}
							}
						...
						if (data.message[0] && data.message[0]["healthIdNumber"]) {
							...
							if (data.message[0].details[0]["message"]) {
								show_message(
									...
									"red",
									data.message[0].details[0]["message"],
									"auth_method",
								);
							}
							frappe.show_alert(
...
let create_abha = function (frm) {
	let d = new frappe.ui.Dialog({
		...
		primary_action(values) {
			if (
				!d.get_value("received_consent") &&
				!d.get_value("patient_consent_attach")
			) {
				frappe.throw({
	...
	print_button.on("click", "button", function () {
		frappe.db
			.get_value("Terms and Conditions", d.get_value("patient_consent"), "terms")
			.then(r => {
...
let set_data_to_form = function (frm, scanned_data, dialog, d) {
	if (scanned_data) {
		...
		for (var k in scanned_data) {
			if (k == "hid" || k == "healthId") {
				frm.set_value("abha_address", scanned_data[k]);
			}
			if (k == "hidn" || k == "healthIdNumber") {
				frm.set_value("abha_number", scanned_data[k]);
			}
			if (!frm.doc.first_name) {
				if (k == "name") {
					frm.set_value("first_name", scanned_data[k]);
				}
			}
			if (!frm.doc.dob) {
				if (dob) {
			...
			if (!frm.doc.email) {
				if (k == "email") {
					frm.set_value("email", scanned_data[k]);
				}
			}
			...
			frappe.call({
				...
				callback: function (data) {
					if (data.message["status"] == false) {
						...
						show_message(
							dialog,
							"ABHA Address is already existing",
							"red",
							"",
							"username",
						);
			...
			frappe.call({
				...
				callback: function (data) {
					if (data.message["txnId"]) {
						show_message(
							dialog,
							"Successfully Resent Aadhaar OTP",
							"green",
							"",
							"otp",
						);
			...
			frappe.call({
				...
				callback: function (data) {
					...
					if (data.message["txnId"] || data.message["token"]) {
						dialog.fields_dict.mobile.$wrapper.find("span").remove();
						dialog.fields_dict.mobile.$wrapper
							.find(".control-input")
							.append(
								`<span class="link-btn" style="display:inline">
