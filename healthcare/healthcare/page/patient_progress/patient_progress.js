...
class PatientProgress {
		...
		this.get_patient_info().then(() => {
			this.get_therapy_sessions_count().then(() => {
				$(".patient-info")
					.empty()
					.append(
						frappe.render_template("patient_progress_sidebar", {
							...
							therapy_sessions_this_month:
								this.therapy_sessions_this_month,
						}),
					);
	...
	update_heatmap_data(date_from) {
		frappe
			.xcall(
				"healthcare.healthcare.page.patient_progress.patient_progress.get_patient_heatmap_data",
				{
					patient: this.patient_id,
	...
	show_therapy_progress() {
		...
		this.create_time_span_filters(
			"render_therapy_progress_chart",
			".therapy-progress",
		);
	}
	...
	show_assessment_results() {
		...
		this.create_time_span_filters(
			"render_assessment_result_chart",
			".assessment-results",
		);
	}
		...
		frappe
			...
			.then(chart => {
				let data = {
					labels: chart.labels,
					datasets: chart.datasets,
					yMarkers: [{ label: "Max Score", value: chart.max_score }],
				};
				...
				if (!chart.labels.length) {
					...
					if (!this.assessment_line_chart) {
						this.assessment_line_chart = new frappe.Chart(parent, {
							...
							tooltipOptions: {
								formatTooltipY: d =>
									__("{0} out of {1}", [d, chart.max_score]),
							},
						});
					} else {
						$(parent).find(".chart-container").show();
	...
	show_therapy_assessment_correlation() {
		...
		assessment.refresh();
		this.create_time_span_filters(
			"render_therapy_assessment_correlation_chart",
			".therapy-assessment-correlation",
		);
	}
	...
	show_assessment_parameter_progress() {
		...
		parameter.refresh();
		this.create_time_span_filters(
			"render_assessment_parameter_progress_chart",
			".assessment-parameter-progress",
		);
	}
