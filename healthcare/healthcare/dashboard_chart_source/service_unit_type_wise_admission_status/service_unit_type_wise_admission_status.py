...
def get(
	...
	company = filters.get("company") if filters and filters.get("company") else frappe.db.get_single_value("Global Defaults", "default_company")
	...
	query = (
		...
		.select(
			...
			Count(frappe.qb.terms.Case().when(ip.status == "Admission Scheduled", 1)).as_("admission_scheduled"),
			Count(frappe.qb.terms.Case().when(ip.status == "Admitted", 1)).as_("admitted"),
			Count(frappe.qb.terms.Case().when(ip.status == "Discharge Scheduled", 1)).as_("discharge_scheduled"),
			Count(frappe.qb.terms.Case().when(ip.status == "Discharged", 1)).as_("discharged"),
		)
		.where(
			(ip.status != "Cancelled")
			& (ip.admission_service_unit_type.isnotnull()
			& (ip.company == company)
		)
		)
	...
	return {
		"labels": labels,
		"datasets": [
			{"name": _("To Admission"), "values": admission_scheduled_ips},
