...
class HealthcareInsuranceClaim(Document):
	def validate(self):
		...
		if self.mode_of_approval == "Manual" and self.status == "Draft" and not self.flags.silent:
			frappe.msgprint(
				_("Manual approval required for Insurance Claim {}").format(self.name),
				alert=True,
				indicator="orange",
			)
	...
	def validate_insurance_policy(self):
		if not self.insurance_subscription:
			frappe.throw(
				_("Patient Insurance Policy is required to create Insurance Claim"),
				title=_("Missing Insurance Policy"),
			)

		if not is_insurance_policy_valid(
			self.insurance_subscription, self.posting_date, self.company
		):  # also checks for valid contract
			frappe.throw(
				_("Patient Insurance Policy {} is not valid as on {}").format(
					frappe.bold(self.insurance_subscription), self.posting_date
				),
	...
	def before_submit(self):
		if self.status not in ["Approved", "Rejected"]:
			frappe.throw(
				_("Only Insurance Claims in Status <b>Approved</b> or <b>Rejected</b> can be submitted"),
				title=_("Not Allowed"),
			)

		if self.status == "Approved" and self.coverage <= 0:
			frappe.throw(
				_("Invalid Coverage Percent {}, cannot submit Insurance Claim as Approved").format(
	...
	def on_submit(self):
		if not self.flags.silent:
			frappe.msgprint(
				_("Insurance Claim {} - {}<br>Discount: {}%, Coverage: {}%").format(
	...
	def on_update_after_submit(self):
		...
		claim_amount_invoiced = sum(
			detail.get("invoice_amount") or 0 for detail in self.insurance_claim_details
		)
		status = "Partially Invoiced" if qty_invoiced < self.qty else "Invoiced"

		self.db_set(
			{"qty_invoiced": qty_invoiced, "claim_amount_invoiced": claim_amount_invoiced, "status": status}
		)
	...
	def before_cancel(self):
		...
		if self.status in ["Invoiced", "Partially Paid", "Paid", "Payment Rejected"]:
			frappe.throw(
				_("Cannot cancel Insurance Claim with Status {}").format(", ".join(not_allowed)),
				title=_("Not Allowed"),
			)
	...
	def set_and_validate_template_details(self):
		...
		if self.template_dt and self.template_dn and self.template_dt != "Appointment Type":
			...
			if not details.get("is_billable"):
				frappe.throw(
					_(
						"Invalid Service Template, Insurance Claim can only be created for Templates marked <b>Is Billable</b>"
					),
					title=_("Not Allowed"),
				)
		...
		if not self.item_code:
			frappe.throw(
				_("Invalid Service Template, Item is required to create Insurance Claim"),
	...
	def set_insurance_coverage(self):
		...
		if not coverage_detail:
			frappe.msgprint(
				_("Insurance Coverage not found for {}.").format(self.item_code), alert=True, indicator="error"
			)
			...
			if coverage_detail.get("valid_till") and getdate(coverage_detail.get("valid_till")) < getdate(
				self.claim_validity_end_date
			):
	...
	def set_insurance_claim_details(self):
		...
		if self.claim_amount <= 0:
			frappe.msgprint(
				_(
					"Error calculating Coverage for Insurance Claim {}. \
				Please verify Coverage for Item and then try saving Insurance Claim again"
				).format(self.name),
				alert=True,
				indicator="error",
			)
...
def make_insurance_claim(
	...
	policy_details = frappe.db.get_value(
		...
		policy,
		["policy_expiry_date", "insurance_coverage_plan"],
		as_dict=True,
	)
	...
	return {"claim": claim.name, "claim_status": claim.status}
