...
class HealthcareInsurancePaymentRequest(Document):
	...
	def before_update_after_submit(self):
		...
		for claim in self.claims:
			if claim.status == "Payment Approved" or claim.approved_amount == claim.claim_amount:
				...
				claim.status in ["Payment Rejected", "Payment Error"]
				or claim.approved_amount < claim.claim_amount
			):
				# Not Approved
				...
				if not claim.payment_error_reason:
					if self.status == "Verified":
						frappe.throw(
							_("<b>Reason for Payment Error</b> is required, please update row #{}").format(claim.idx),
							title=_("Reason "),
						)
					else:
						frappe.msgprint(
				...
				frappe.throw(
					_("Approved Amount connot be more than Claim Amount, please update row #{}").format(
						claim.idx
					),
	...
	def validate_existing_payment_requests(self):
		...
		payment_request_items = frappe.db.get_all(
			"Healthcare Insurance Payment Request Item",
			{
				"parent": ["!=", self.name],
		...
		if payment_request_items:
			frappe.throw(
				_("Payment Request already submitted for Insurance Claims<br>{}").format(
					"<br> ".join(
						[
							f"{claim.insurance_claim} ({claim.status}),\
					Payment Request: {get_link_to_form(self.doctype, claim.parent)}"
							for claim in payment_request_items
						]
					)
				),
	...
	def update_linked_insurance_claim(self, claim):
		...
		claim_status_map = {"Submitted": "Payment Requested", "Cancelled": "Payment Error"}
	...
	def get_claims(self):
		...
		""".format(
				col="si.posting_date" if self.posting_date_based_on == "Sales Invoice" else "cl.posting_date"
			),
			{
...
def create_payment_entry(doc):
	...
	payment_entry.remarks = _(
		"Payment Entry against Insurance Claims {} via Healthcare Insurance Payment Request {}"
	).format(", ".join([claim.get("insurance_claim") for claim in doc.claims]), doc.name)
