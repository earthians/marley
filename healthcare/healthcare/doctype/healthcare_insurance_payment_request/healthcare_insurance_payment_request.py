...
class HealthcareInsurancePaymentRequest(Document):
	...
	def get_claim_items(self):
		filters = {"insurance_company": self.insurance_company, "docstatus": 1, "status": "Invoiced"}

		if self.posting_date_type == "Claim Posting Date":
			filters.update({"claim_posting_date": (">=", self.from_date)})
		else:
			filters.update(
				{
...
def create_payment_entry(doc):
	...
	insurance_company_customer = frappe.db.get_value(
		"Healthcare Insurance Company", doc.insurance_company, "customer"
	)
