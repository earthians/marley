...
class HealthcareInsuranceSubscription(Document):
	def validate(self):
		...
		if not has_active_contract(self.insurance_company):
			frappe.throw(
				_("No active contracts found for Insurance Company {0}").format(self.insurance_company)
			)
	...
	def validate_subscription_overlap(self):
		insurance_subscription = frappe.db.exists(
		...
		if insurance_subscription:
			frappe.throw(
				_(
					"Patient {0} already has an active insurance subscription {1} with the coverage plan {2} for this period"
				).format(
					frappe.bold(self.patient),
					get_link_to_form("Healthcare Insurance Subscription", insurance_subscription),
					frappe.bold(self.insurance_coverage_plan),
				),
				title=_("Duplicate"),
			)
	...
	def validate_policy_number(self):
		...
		if insurance_subscription:
			frappe.throw(
				_(
					"Patient {0} already has an insurance subscription {1} with the same Policy Number {2}"
				).format(
					frappe.bold(self.patient),
					get_link_to_form("Healthcare Insurance Subscription", insurance_subscription),
...
def is_insurance_policy_valid(subscription, on_date=None, company=None):
	...
	"""
	policy_expiry = frappe.db.get_value(
		"Healthcare Insurance Subscription", subscription, ["policy_expiry_date"]
	)
...
def get_insurance_price_lists(insurance_subscription, company):
	...
	"""
	price_lists = {}
	coverage_plan, insurance_company = frappe.db.get_value(
		"Healthcare Insurance Subscription",
		insurance_subscription,
		["insurance_coverage_plan", "insurance_company"],
	)
	if coverage_plan:
		plan_price_list = frappe.db.get_value(
			"Healthcare Insurance Coverage Plan", coverage_plan, "price_list"
		)
		if plan_price_list:
	...
	if insurance_company and company:
		default_price_list = frappe.db.get_value(
			...
			{"insurance_company": insurance_company, "company": company},
			"default_price_list",
		)
