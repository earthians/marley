...
class HealthcareInsuranceSubscription(Document):
	...
	def validate_insurance_company(self):
		...
		if not contract:
			frappe.throw(
				_("There is no valid contract with this Insurance Company {0}").format(self.insurance_company)
			)
	...
	def set_title(self):
		self.title = _("{0} with {1}").format(
			self.patient_name or self.patient, self.insurance_company_name or self.insurance_company
		)
