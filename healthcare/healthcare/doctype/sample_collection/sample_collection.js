...
frappe.ui.form.on("Sample Collection", {
	...
	refresh: function (frm) {
		...
		frm.fields_dict.observation_sample_collection.grid
			.add_custom_button(__("Mark Collected"), () => {
				selected_child =
					frm.fields_dict.observation_sample_collection.grid.get_selected_children();
				if (selected_child.length > 0) {
					frappe.confirm(
						...
						function () {
							frappe.dom.freeze(
								__("Creating Observations! Please Wait..."),
							);
							frappe.call({
...
frappe.ui.form.on("Observation Sample Collection", {
	sample: function (frm, cdt, cdn) {
		...
		frappe.db
			.get_value("Lab Test Sample", row.sample, [
				"sample_type",
				"sample_uom",
			...
			.then(r => {
				...
				frappe.model.set_value(
					...
					cdn,
					"container_closure_color",
					values.container_closure_color,
				);
			});
	},
	...
	if (frm.doc.observation_sample_collection) {
		...
		$.each(frm.doc.observation_sample_collection, function (k, val) {
			if (val.has_component === 0 && val.specimen) {
				...
				val.has_component === 1 &&
				JSON.parse(val.component_observations).length > 0
			) {
