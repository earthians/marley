...
frappe.ui.form.on("Practitioner Schedule", {
	refresh: function (frm) {
		frm.fields_dict["time_slots"].grid.wrapper.find(".grid-add-row").hide();
		frm.fields_dict["time_slots"].grid.add_custom_button(
			__("Add Time Slots"),
			() => {
				let d = new frappe.ui.Dialog({
					fields: [
						...
						},
						{
							...
							label: __("Create Slots"),
							fieldtype: "Check",
							default: 1,
						},
						{
							fieldname: "maximum_appointments",
							label: __("Maximum Number of Appointments (Nos)"),
							fieldtype: "Int",
							mandatory_depends_on: "eval:doc.create_slots==0",
									...
									while (cur_time < end_time) {
										...
										if (to_time <= end_time) {
											if (frm.doc.time_slots) {
												frm.doc.time_slots.forEach(
													function (slot) {
														if (slot.day == week_day) {
															let slot_from_moment =
																moment(
																	slot.from_time,
																	"HH:mm:ss",
																);
									...
									if (add_slots_to_child && frm.doc.time_slots) {
										frm.doc.time_slots.forEach(function (slot) {
											if (slot.day == week_day) {
												...
												if (
													cur_time.isSame(slot_from_moment) ||
													cur_time.isBetween(
														slot_from_moment,
														slot_to_moment,
													) ||
													end_time.isSame(slot_to_moment) ||
													end_time.isBetween(
														slot_from_moment,
														slot_to_moment,
													)
...
var set_maximum_no_of_appointments = function (d) {
	if (!d.get_value("create_slots")) {
		...
		if (d.get_value("from_time") && d.get_value("to_time")) {
			interval =
				((moment(d.get_value("from_time"), "HH:mm:ss") -
					moment(d.get_value("to_time"), "HH:mm:ss")) /
					60000) |
				0;
