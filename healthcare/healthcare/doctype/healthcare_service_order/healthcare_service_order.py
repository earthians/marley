...
from erpnext.healthcare.doctype.healthcare_insurance_claim.healthcare_insurance_claim import (
...
class HealthcareServiceOrder(Document):
	...
	def before_cancel(self):
		...
		if self.status in not_allowed:
			frappe.throw(
				_("You cannot Cancel Service Order in {} status").format(", ".join(not_allowed)),
				title=_("Not Allowed"),
			)
	...
	def set_order_details(self):
		if not self.template_dt and not self.template_dn:
			frappe.throw(
				_("Order Template Type and Order Template are mandatory to create Healthcare Service Order"),
