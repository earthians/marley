					...
					if (cur_dialog.fields_dict.service_unit_type.value) {
						frappe.db
							.get_value(
								...
								cur_dialog.fields_dict.service_unit_type.value,
								"overlap_appointments",
							)
							.then(r => {
								if (r.message.overlap_appointments) {
									cur_dialog.set_df_property(
				...
				const dialog = new frappe.ui.Dialog({
					...
					fields: [
								...
								if (cur_dialog) {
									if (
										...
										frappe.db
											.get_value(
												"Healthcare Service Unit Type",
												cur_dialog.fields_dict.service_unit_type
													.value,
												"overlap_appointments",
											)
											.then(r => {
						...
						},
						{
							...
							reqd: false,
							depends_on:
								"eval:!doc.is_group && doc.service_unit_type != ''",
							hidden: true,
						},
						{
							fieldtype: "Link",
						...
						return frappe.call({
							...
							callback: function (r) {
								if (!r.exc && r.message) {
									...
									].treeview.tree.load_children(node, true);

									frappe.show_alert({
										message: __("{0} Service Units created", [
											vals.count - r.message.length,
										]),
										indicator: "green",
									});
								} else {
									frappe.msgprint(
										__("Could not create Service Units"),
									);
								}
							},
							freeze: true,
							freeze_message: __("Creating {0} Service Units", [
								vals.count,
							]),
						});
					},
