				...
				.then(r => {
					frm.set_value({
						due_date: frappe.datetime.add_days(
							...
							r.message.insurance_claim_credit_days,
						),
					});
				});
		}
				...
				primary_action: () => {
					...
					coverages.forEach(coverage => {
						frappe.model.set_value(coverage.doctype, coverage.name, {
							...
							approved_amount:
								values.status == "Approved" ? coverage.claim_amount : 0,
							rejected_amount:
								values.status == "Approved" ? 0 : coverage.claim_amount,
...
frappe.ui.form.on("Insurance Claim Coverage", {
	before_coverages_remove: function (frm) {
		if (frm.doc.docstatus != 0) {
			frappe.throw(
				__(
					"Not Allowed to delete coverage after submission, please update the Coverage status instead",
				),
			);
		}
	},
