...
class InsuranceClaim(Document):
	...
	def before_update_after_submit(self):
		...
		for coverage in self.coverages:
			if (
				coverage.status == "Payment Approved" or coverage.approved_amount == coverage.coverage_amount
			):
				# Approved
				...
				coverage.status in ["Payment Rejected", "Claim Error"]
				or coverage.approved_amount < coverage.coverage_amount
			):
				# Not Approved
				...
				if not coverage.payment_error_reason:
					if self.status == "Submitted":
						frappe.throw(
							_("<b>Reason for Claim Error</b> is required, please update row #{}").format(coverage.idx),
							title=_("Reason "),
						)
					else:
						frappe.msgprint(
	...
	def update_linked_insurance_coverage(self, coverage):
		...
		coverage_status_map = {"Submitted": "Claim Processing", "Cancelled": "Claim Error"}
...
def create_payment_entry(doc):
	...
	payment_entry.remarks = _(
		"Payment Entry against Insurance Coverages {} via Insurance Claim {}"
	).format(
