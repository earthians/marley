...
class HealthcareServiceInsuranceCoverage(Document):
	...
	def validate_overlaps(self):
		conditions = (
			"""is_active = 1 and name != {name} and ifnull(insurance_coverage_plan, '') = {plan}""".format(
		...
		if self.valid_from and self.valid_till:
			...
			conditions += """ and (valid_till >= {valid_from} or valid_from = {valid_from})""".format(
				valid_from=frappe.db.escape(self.valid_from)
			)

		elif not self.valid_from and self.valid_till:
			conditions += """ and (valid_from <= {valid_till} or valid_till = {valid_till}""".format(
				valid_till=frappe.db.escape(self.valid_till)
			)

		conditions += """ and ifnull(item_code, '') = {item_code}""".format(
			item_code=frappe.db.escape(self.item_code)
		)

		conditions += (
			""" and ifnull(template_dt, '') = {dt} and ifnull(template_dn, '') = {dn} """.format(
		...
		if overlap:
			frappe.throw(
				_("Coverage overlaps with {}").format(get_link_to_form(self.doctype, overlap[0].name)),
...
def get_insurance_coverage(
	item_code, template_dt=None, template_dn=None, on_date=None, coverage_plan=None
):

	conditions = """ifnull(is_active, 0) = 1 and
		ifnull(insurance_coverage_plan, '') = {}""".format(
		frappe.db.escape(coverage_plan or "")
	)

	conditions += """ and ('{}' between
		ifnull(valid_from, '2000-01-01') and ifnull(valid_till, '2500-12-31'))""".format(
		getdate(on_date) or getdate()
	)
	...
	conditions += (
		""" or (ifnull(template_dt, '') = {dt} and ifnull(template_dn, '') = {dn}) )""".format(
