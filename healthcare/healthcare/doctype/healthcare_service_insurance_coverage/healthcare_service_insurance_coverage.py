...
class HealthcareServiceInsuranceCoverage(Document):
	...
	def validate_service_overlap(self):
		...
		service_insurance_coverages = frappe.get_list(
			"Healthcare Service Insurance Coverage", filters=filters, or_filters=or_filters
		)

		if service_insurance_coverages:
			frappe.throw(
				_("Service or Item coverage overlapping with coverage {0} under the coverage plan {1}").format(
					get_link_to_form(self.doctype, service_insurance_coverages[0].name),
	...
	def get_filters(self):
		...
		or_filters = {"start_date": ["<=", self.start_date], "end_date": [">=", self.end_date]}
...
def get_service_insurance_coverage_details(
	...
	insurance_subscription = frappe.db.get_value(
		...
		["name", "healthcare_insurance_coverage_plan", "insurance_company"],
		as_dict=True,
	)
	...
	if insurance_subscription and is_valid_insurance(insurance_subscription, valid_date):
		...
		if any((coverage["healthcare_service_template"] == service) for coverage in coverage_list):
			...
			coverage, discount, is_auto_approval = get_insurance_coverage_details(
				coverage_plan, service_item=service_item
			)
			insurance_details = frappe._dict(
				{"is_auto_approval": is_auto_approval, "discount": discount, "coverage": coverage}
			)
			...
			if medical_code:
				if any((coverage["medical_code"] == medical_code) for coverage in coverage_list):
					coverage, discount, is_auto_approval = get_insurance_coverage_details(
						coverage_plan, medical_code=medical_code
					)
					insurance_details = frappe._dict(
						{"is_auto_approval": is_auto_approval, "discount": discount, "coverage": coverage}
					)
				...
				if item_group:
					if any((coverage["item_group"] == item_group) for coverage in coverage_list):
						coverage, discount, is_auto_approval = get_insurance_coverage_details(
							coverage_plan, item_group=item_group
						)
						insurance_details = frappe._dict(
							{"is_auto_approval": is_auto_approval, "discount": discount, "coverage": coverage}
						)
...
def get_insurance_coverage_details(
	...
	if healthcare_service_coverage:
		insurance_coverage = frappe.db.get_value(
			...
			["coverage", "discount", "claim_approval_mode"],
			as_dict=True,
		)
...
def is_valid_insurance(insurance_subscription, posting_date):
	if frappe.db.exists(
		...
		if frappe.db.exists(
			"Healthcare Insurance Subscription",
			{"name": insurance_subscription.name, "subscription_expiry": (">=", getdate(posting_date))},
		):
			return True
	return False
