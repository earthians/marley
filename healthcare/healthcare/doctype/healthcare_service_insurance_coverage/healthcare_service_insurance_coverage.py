...
class HealthcareServiceInsuranceCoverage(Document):
	...
	def validate_service_overlap(self):
		...
		if service_insurance_coverages:
			frappe.throw(
				_("Service/Item activated  this coverage plan {0}").format(
...
def get_service_insurance_coverage_details(
	...
	if insurance_subscription and valid_insurance(
		insurance_subscription.name, insurance_subscription.insurance_company, valid_date
	):
		if insurance_subscription.healthcare_insurance_coverage_plan:
			healthcare_service_coverage_list = frappe.get_list(
			...
			if healthcare_service_coverage_list:
				if any(
					(healthcare_service_coverage["healthcare_service_template"] == service)
					for healthcare_service_coverage in healthcare_service_coverage_list
				):
					coverage, discount, is_auto_approval = get_insurance_coverage_details(
						insurance_subscription.healthcare_insurance_coverage_plan, service=service
					)
					insurance_details = frappe._dict(
						{"is_auto_approval": is_auto_approval, "discount": discount, "coverage": coverage}
					)
				elif any(
					(healthcare_service_coverage["item"] == service_item)
					for healthcare_service_coverage in healthcare_service_coverage_list
				):
					coverage, discount, is_auto_approval = get_insurance_coverage_details(
						insurance_subscription.healthcare_insurance_coverage_plan, service_item=service_item
					)
					insurance_details = frappe._dict(
						{"is_auto_approval": is_auto_approval, "discount": discount, "coverage": coverage}
					)
					...
					if medical_code:
						if any(
							(healthcare_service_coverage["medical_code"] == medical_code)
							for healthcare_service_coverage in healthcare_service_coverage_list
						):
							coverage, discount, is_auto_approval = get_insurance_coverage_details(
								insurance_subscription.healthcare_insurance_coverage_plan, medical_code=medical_code
							)
							insurance_details = frappe._dict(
								{"is_auto_approval": is_auto_approval, "discount": discount, "coverage": coverage}
							)
						...
						if item_group:
							if any(
								(healthcare_service_coverage["item_group"] == item_group)
								for healthcare_service_coverage in healthcare_service_coverage_list
							):
								coverage, discount, is_auto_approval = get_insurance_coverage_details(
									insurance_subscription.healthcare_insurance_coverage_plan, item_group=item_group
								)
								insurance_details = frappe._dict(
									{"is_auto_approval": is_auto_approval, "discount": discount, "coverage": coverage}
								)
...
def get_insurance_coverage_details(
