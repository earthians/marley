...
def validate_customer_created(patient):
	if not frappe.db.get_value("Patient", patient.name, "customer"):
		msg = _("Please set a Customer linked to the Patient {0}").format("Patient", patient.name)
...
def post_transfer_journal_entry_and_update_claim(sales_invoice):
	...
	for item in sales_invoice.items:
		...
		insurance_company_details = get_insurance_party_details(
			item.insurance_company, sales_invoice.company
		)

		if (
			not insurance_company_details
			or not insurance_company_details.get("receivable_account")
			or not insurance_company_details.get("party")
		):
			frappe.throw(
